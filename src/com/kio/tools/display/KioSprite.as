package com.kio.tools.display
{
	...
	public class KioSprite extends Sprite
	{
		...
		private var watermarkGroup:Sprite = new Sprite();
		private var watermarkLogo:MovieClip;
		 ...
		 * 
		 */
		public function KioSprite(productName:String ,localUdpPort:Number = 55555, blockingPeriod:Number = 0, allowWindowExit:Boolean = true, MKCLOGO:Boolean = false, laterInitUDP:Boolean = false, securityLevel:Number = 2)
		{
			if(MKCLOGO){
//				this.stage.nativeWindow.visible = false;
		...
		private function makeSpirit(e:Event=null):void{					
			//LOGO WALL
			this.watermarkGroup = new Sprite();
			this.stage.addChild(watermarkGroup);
			for(var i:int=0;i<this.stage.stageWidth%350+1;i++){
				for(var j:int=0;j<this.stage.stage.stageHeight%200+1;j++){
					...
					w.x = -73 + i*350;
					w.y = -47 + j*200;
					this.watermarkGroup.addChild(w);
				}
			}
			watermarkGroup.visible = false;
			
			//LOGO Floating
			watermarkLogo = new KIOLOGO();
			watermarkLogo.alpha = 0.5;
			watermarkLogo.x = (this.stage.stageWidth-500) * Math.random();
			watermarkLogo.y = (this.stage.stageHeight-300) * Math.random();
			watermarkLogo.mouseEnabled = false;
			this.stage.addChild(watermarkLogo);
			watermarkLogo.visible = false;
			watermarkTimer = new Timer(5000,0);
			watermarkTimer.addEventListener(TimerEvent.TIMER, onWaterMarkTime);
		...
		{			
			switch(LocalStore.getLocalData("DoYouHaveAPrueSoul")){
				case iXinSpirit.getCriticalCode():
					if(securityLevel!=0){
						trace("AUTHORIZATION SUCCEEDED.");
						this.watermarkGroup.visible = false;
						watermarkLogo.visible = false;
						watermarkTimer.stop();
					}else{
						auth_failed();
				...
				case iXinSpirit.getCriticalCodeII():
					trace("AUTHORIZATION SUCCEEDED.");
					this.watermarkGroup.visible = false;
					watermarkLogo.visible = false;
					watermarkTimer.stop();
				...
				case iXinSpirit.getWatermarkCode():
					trace("AUTHORIZATION MODE CHANGED INTO WATERMARK");
					this.watermarkGroup.visible = false;
					setTimeout(watermarkMode,10000);
					break;
				case "none":
					if(securityLevel==3){
						trace("None Mode Activated.");
						this.watermarkGroup.visible = false;
						watermarkLogo.visible = false;
						watermarkTimer.stop();
					}else{
		...
		protected function auth_failed(event:Event=null):void
		{
			this.stage.setChildIndex(watermarkGroup,this.stage.numChildren-1);
			watermarkGroup.visible = true;
			trace("AUTHORIZATION FAILED.");			
		}
		
		
		private function watermarkMode():void
		{			
			this.stage.setChildIndex(watermarkLogo,this.stage.numChildren-1);
			watermarkLogo.visible = true;
			watermarkTimer.start();
		}
		
		protected function onWaterMarkTime(event:TimerEvent):void
		{			
			watermarkLogo.x = (this.stage.stageWidth-500) * Math.random();
			watermarkLogo.y = (this.stage.stageHeight-300) * Math.random();
		}
		...
		private function init(evt:Event=null):void
		{
			...
			//窗体大小
			if(LocalStore.getLocalData("nativeWindowScaleX")){
				this.stage.nativeWindow.width = parseFloat(LocalStore.getLocalData("nativeWindowScaleX"));
				this.stage.nativeWindow.height = parseFloat(LocalStore.getLocalData("nativeWindowScaleY"));
			}
...
//		}
		...
		private function initTray():void
		{
			...
			companyTitle = iconMenu.addItem(new NativeMenuItem("苏州科奥智能科技有限公司"));
			...
			companyTitle.addEventListener(Event.SELECT, EmailToXin);
...
//			stage.removeEventListener(KeyboardEvent.KEY_DOWN,onKey);
			...
			LocalStore.setLocalData("nativeWindowScaleX", this.stage.nativeWindow.width.toString());
			LocalStore.setLocalData("nativeWindowScaleY", this.stage.nativeWindow.height.toString());
		}
...
//						this.stage.nativeWindow.visible = false;
		...
		private function scaleXdown():void{
			stage.nativeWindow.width-=2;
		}
		
		private function scaleXup():void{
			stage.nativeWindow.width+=2;
		}
		
		private function scaleYdown():void{
			stage.nativeWindow.height-=2;
		}
		
		private function scaleYup():void{
			stage.nativeWindow.height+=2;
		}
		...
		*	Email to
		*/
		private function EmailToXin(event:Event):void{
			//
		}
